...
public struct SortedMap<K: Enumerable, V: Hashable & Comparable>: RandomAccessCollection {
    
    private var keys: Keys = .init()
...
public extension SortedMap {
    ...
    subscript(key: Key) -> Value? {
        ...
        set {
            if let value: Value = newValue, value.isNotEmpty {
                self.keys += key
                self.map[key] = value
            } else {
                self.keys -= key
