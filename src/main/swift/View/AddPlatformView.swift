...
struct AddPlatformView: AddPlatformProtocol {
    ...
    @ViewBuilder
    private func PhysicalView(_ system: SystemBuilder) -> some View {
        WrapperView(system.physicalBuilder) { physical in
            Section("Physical") {
...
fileprivate class AddPlatform: ObservableObject {
    ...
    let used: SystemBuilders
    let isNavigationLinkDisabled: Bool

    init(_ builder: GameBuilder, _ system: SystemBuilder?) {
        self.system = system
        self.formats = builder.tags[system]
        self.used = builder.tags.platforms.used
        self.isNavigationLinkDisabled = system != nil
    }
    
    var count: Int { self.formats.quantity }
...
fileprivate protocol AddPlatformProtocol: Gameopticable {
    ...
    typealias Element = Platforms.Member // (SystemBuilder, Formats)
}
...
fileprivate extension AddPlatformProtocol {
    ...
    func update(_ format: FormatBuilder) -> Void {
        if let system: SystemBuilder = self.system {
            let element: PlatformBuilder = .init(system, format)
            if contains(element.format) {
    ...
    func get(_ system: SystemEnum) -> SystemBuilders {
    ...
    var isDoneDisabled: Bool {
        let isUnchanged: Bool = self.builder.tags[system] == self.object.formats
        let isNull: Bool = self.system == nil
        let isEmpty: Bool = self.object.formats.isVacant
